...
view: data_intelligence_ar {
  ...
  measure: Sales_Total_DSO {
    ...
    sql:
      CASE
        when CAST(${TABLE}.PostingDateInTheDocument_BUDAT as Date)<= CAST('2022-09-01' as Date) and CAST(${TABLE}.PostingDateInTheDocument_BUDAT as Date)>= DATE_SUB(CAST('2022-09-01' as Date),INTERVAL {% parameter Day_Sales_Outstanding %} MONTH )
      THEN ${Sales_Global_Currency}
      END;;
    #sql:
    #CASE
    #when CAST(${TABLE}.PostingDateInTheDocument_BUDAT as Date)<= CAST('2022-09-01' as Date) and CAST(${TABLE}.PostingDateInTheDocument_BUDAT as Date)>= DATE_SUB(${Current_Fiscal_Date_date},INTERVAL {% parameter Day_Sales_Outstanding %} MONTH )
    #THEN ${Sales_Global_Currency}
    #END;;
    }
